...
async function checkReviews (robot, context) {
  ...
  const reviewRequests = await context.github.pullRequests.getReviewRequests(pullRequest).data

  const isOk = reviewRequests.users.length === 0 && reviewRequests.teams.length === 0
